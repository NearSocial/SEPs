...
1. Adjacent to root level socialDB accounts, create a new top account called `groups.near`. However, since this account name has already been claimed, I have slightly rearranged the same letters to use the spanish translation: `grupos.near`. For convenience, gateway operators could add a redirect from `gateway.com/near/groups.near` -> `gateway.com/near/grupos.near` if desired.
